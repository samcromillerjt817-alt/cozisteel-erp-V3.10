...
export async function POST(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    if (!paidAt) return badRequest('Data de pagamento inválida — use aaaa-mm-dd ou dd/mm/aaaa')

    const updated = await financialAccountService.registerPayment(id, data.amount, paidAt, data.notes, user.id)
    return ok(updated)
  } catch (error) {
